...
df = read.csv("Data_Kelompok K.csv",header=TRUE,sep=';')
head(df)
...
by_kota1 <- by_kota %>% group_by(kota) %>%
  summarise(ulasan=mean(ulasan),
            dijual = mean(produk_dijual),
            .groups = 'drop'
  )
by_kota1
...
ui <- dashboardPage(
  dashboardHeader(title="Dashboard Kelompok 5",titleWidth = 250),
                                     ...
                                     tabBox(id="visualisasi", width=NULL, height = "360px", selected = "Summary",
                                            ...
                                            tabPanel("Cleveland Plot",
                                                     selectInput(inputId="clev",
                                                                     ...
                                                                     choices=list("Ulasan"=1,"Produk Dijual"=2
                                                                                  ),
                                                                     selected = 1),
...
",
  ...
  output$anggota = renderText({
    "Nama Anggota Kelompok 5:\n 1. Dimyadi Suhartono (162112133014)\n 2. Juan Alberto Galih Leo (162112133027)\n 3.  Farras Alfiyyah (162112133086)\n 4. Kridha Mei Wulandari (162112133091)\n 5. Ratu Noor Hasannah (162112133112)"
  })
